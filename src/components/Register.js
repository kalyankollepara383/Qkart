...
class Register extends React.Component {
 ...
 hideloading=()=>{
  ...
  performAPICall = async () => {
    ...
    this.hideloading()
    if(this.validateResponse(flag,await (await response).json())){
      return response
    }
  ...
  validateInput = () => {
    ...
    if(this.state.username.length===0){
      message.error('username is required feild')
      return false
    }
    else if(this.state.password.length===0){
      message.error('Password is required feild')
      return false
    }
    else if(this.state.username<6){
      message.error('username must be greater than 6')
      return false
    }
    else if(this.state.password>32){
      message.error('username must be less than 32')
      return false
    }
    else if(this.state.password<6){
      message.error('password must be greater than 6')
    }
    else if(this.state.password>32){
      message.error('password must be less than 32')
      return false
    }
   ...
   */
  register = async () => {
    //message.info("Register method is not implemented");
