...
class Search extends React.Component {
   ...
   */
  getProducts = async () => {
    const apicall=await this.performAPICall();
    console.log(apicall)
    if(apicall){
      this.products=apicall;
  ...
  search = (text) => {
      ...
      const filteredproductsarray=this.products.filter((values)=>{
        if(values.category.toUpperCase().includes(text.toUpperCase()) || values.name.toUpperCase().includes(text.toUpperCase()))
        {
          return values
        }
        ...
        <Product
          ...
          addToCart={() => {
            if (this.state.loggedIn) {
              ...
              return this.props.history.push(routes.Loginpage);
            }
      ...
      <>
        ...
        <Header history={this.props.history}>
          {/* TODO: CRIO_TASK_MODULE_PRODUCTS - Display search bar in the header for Products page */}
            <Input.Search  placeholder="search the products" onSearch={this.search.bind(this)} onChange={this.debounceSearch} enterButton ></Input.Search>
