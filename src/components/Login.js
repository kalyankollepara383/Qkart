...
class Login extends React.Component {
  ...
  performAPICall = async () => {
    ...
    try{
      const response= await getApiresponse(`${config.endpoint}/auth/login`,"POST",headers,JSON.stringify(data))
      if(this.validateResponse(false,response)){
        return  response
      }
    }
    catch(error){
      if(this.validateResponse(true,undefined)){
        return undefined
      }
  ...
  login = async () => {
   // message.info("Login method is not implemented")
