...
class Checkout extends React.Component {
  ...
  deleteAddress = async (addressId) => {
    ...
    if (this.validateResponse(errored, response,"address")) {
      message.info("Address Deleted Succssfully")
      await this.getAddresses()
    }
  };
  ...
  checkout = async () => {
    ...
    if (this.validateResponse(errored, response, "checkout")) {
        ...
        message.success("Order places Successfully")
        const totalcost=await this.cartRef.current.calculateTotal();
        const bal=localStorage.getItem('Balance')
        localStorage.setItem('Balance',bal-totalcost)
        this.props.history.push(routes.Thankspage)
    }
  };
  ...
  order = async () => {
    ...
    const address=await this.getAddresses();
      if(localStorage.getItem('Balance')<totalcost){
          message.error("Doesn't have enough Balance")
      }
      else{
            this.checkout();
      }
  ...
  async componentDidMount() {
      ...
      await this.getAddresses();
      const cartitems=await this.cartRef.current.getCart()
      if(username && token){
        if(cartitems.length===0){
          message.info("cart is Empty")
           return this.props.history.push(routes.Productspage)
        }
        else{
          this.setState({
        ...
        message.info("user must be logged in first ")
      }
  }
  /**
