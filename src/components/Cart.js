...
export default class Cart extends React.Component {
  ...
  if(this.props.checkout){
    ...
    return <InputNumber min={1} max={10} defaultValue={item.qty} onChange={(value)=>{this.pushToCart(item.productId,value,false)}}></InputNumber>
  }}
  else if(window.location.pathname==routes.Checkoutpage){
     return(
  ...
  handlecheckout=()=>{
      if(this.state.items.length===0){
        ...
        return this.props.history.push(routes.Checkoutpage)
      }
  }
  ...
  render() {
    return (
      ...
      >
        ...
        {!this.props.checkout&& <Button shape="square"type="warning" icon={<ShoppingCartOutlined />} onClick={this.handlecheckout}>Checkout</Button>}
